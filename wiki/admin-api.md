...
if (approved) {
  await sendEmail({
    to: user.email,
    subject: 'Your WESMUN Account Has Been Approved',
    body: 'You can now log in to the WESMUN system...',
  });
} else {
  await sendEmail({
    to: user.email,
    subject: 'WESMUN Account Registration Status',
    body: 'Unfortunately, your registration could not be approved...',
  });
}
```
...
```javascript
// After approval, notify external systems
